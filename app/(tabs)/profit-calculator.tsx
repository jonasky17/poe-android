...
export default function ProfitCalculatorScreen() {
  ...
  const { divineChaosRate, loading: isRateLoading, error: rateError } = usePoeData(CURRENT_LEAGUE);
  ...
  const [draftInvestment, setDraftInvestment] = useState<Omit<InvestmentEntry, 'id'>>(EMPTY_INVESTMENT_ENTRY);
  const [draftLoot, setDraftLoot] = useState<Omit<InvestmentEntry, 'id'>>(EMPTY_INVESTMENT_ENTRY);
  const [draftMap, setDraftMap] = useState<Omit<MapEntry, 'id'>>(EMPTY_MAP_ENTRY);
  ...
  const [renameComputationId, setRenameComputationId] = useState<string | null>(null);
  ...
  const chaosPerDivine = Math.max(0, toNumber(state.exchangeRateChaosPerDivine));

  const investmentRows = useMemo<ComputedInvestmentRow[]>(() => {
    return state.investments.map((entry) => {
      const parsedRatioItemAmount = Math.max(0, toNumber(entry.ratioItemAmount));
      const parsedRatioCostAmount = Math.max(0, toNumber(entry.ratioCostAmount));
      const parsedQuantity = Math.max(0, toNumber(entry.quantity));
      const totalInRatioCurrency = parsedRatioItemAmount > 0
        ? (parsedQuantity / parsedRatioItemAmount) * parsedRatioCostAmount
        : 0;
      const totalChaos = entry.ratioCurrency === 'divine'
        ? totalInRatioCurrency * chaosPerDivine
        : totalInRatioCurrency;
      const totalDiv = entry.ratioCurrency === 'divine'
        ? totalInRatioCurrency
        : chaosPerDivine > 0 ? totalInRatioCurrency / chaosPerDivine : 0;
  ...
  const tableTotals = useMemo(() => {
    const totalChaos = investmentRows.reduce((sum, entry) => sum + entry.totalChaos, 0);
    const totalDiv = investmentRows.reduce((sum, entry) => sum + entry.totalDiv, 0);
    const totalQuantity = investmentRows.reduce((sum, entry) => sum + entry.parsedQuantity, 0);
  ...
  const lootTotals = useMemo(() => {
    const totalChaos = lootRows.reduce((sum, entry) => sum + entry.totalChaos, 0);
    const totalDiv = lootRows.reduce((sum, entry) => sum + entry.totalDiv, 0);
    const totalQuantity = lootRows.reduce((sum, entry) => sum + entry.parsedQuantity, 0);
  ...
  const mapRows = useMemo<ComputedMapRow[]>(() => {
    return state.maps.map((entry) => {
      const parsedCompletionMinutes = Math.max(0, Math.floor(toNumber(entry.completionMinutes)));
      const parsedCompletionSeconds = Math.min(59, Math.max(0, Math.floor(toNumber(entry.completionSeconds))));
      const parsedQuantity = Math.max(0, toNumber(entry.quantity));
      const totalSeconds = ((parsedCompletionMinutes * 60) + parsedCompletionSeconds) * parsedQuantity;
  ...
  const mapTotals = useMemo(() => {
    const totalSeconds = mapRows.reduce((sum, entry) => sum + entry.totalSeconds, 0);
    const totalQuantity = mapRows.reduce((sum, entry) => sum + entry.parsedQuantity, 0);
  ...
  const totalInvestmentDiv = tableTotals.totalDiv + (chaosPerDivine > 0 ? additionalInvestment / chaosPerDivine : 0);
  ...
  const calc = useMemo(() => {
    ...
    const roi = totalInvestmentChaos > 0 ? (profit / totalInvestmentChaos) * 100 : 0;
  ...
  }, [lootTotals.totalChaos, lootTotals.totalDiv, totalInvestmentChaos, chaosPerDivine]);

  const hasMapData = mapRows.length > 0;
  const profitPerHourChaos = mapTotals.totalSeconds > 0
    ? (calc.profit * 3600) / mapTotals.totalSeconds
    : 0;
  const profitPerHourDiv = chaosPerDivine > 0 ? profitPerHourChaos / chaosPerDivine : 0;
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const parsedInvestments = Array.isArray(parsed.investments) ? parsed.investments : [];
        const parsedLoots = Array.isArray(parsed.loots) ? parsed.loots : [];
        const parsedMaps = Array.isArray(parsed.maps) ? parsed.maps : [];

        const normalizedInvestments: InvestmentEntry[] = parsedInvestments.map((entry) => {
          const maybeLegacy = entry as Partial<InvestmentEntry> & { ratioPerItem?: string };

          return {
            id: maybeLegacy.id ?? `${Date.now()}-${Math.random()}`,
            itemName: maybeLegacy.itemName ?? '',
            ratioItemAmount: maybeLegacy.ratioItemAmount ?? '1',
            ratioCostAmount: maybeLegacy.ratioCostAmount ?? maybeLegacy.ratioPerItem ?? '',
            ratioCurrency: maybeLegacy.ratioCurrency === 'chaos' ? 'chaos' : 'divine',
        ...
        const normalizedMaps: MapEntry[] = parsedMaps.map((entry) => {
          ...
          const legacyMinutesRaw = maybeLegacy.completionMinutes ?? maybeLegacy.timeMinutes ?? '';
          const legacySecondsRaw = maybeLegacy.completionSeconds ?? maybeLegacy.timeSeconds ?? '';
          const hasExplicitSeconds = maybeLegacy.completionSeconds !== undefined || maybeLegacy.timeSeconds !== undefined;
          ...
          if (legacyMinutesRaw !== '' || legacySecondsRaw !== '') {
            if (hasExplicitSeconds) {
              ...
              const totalLegacySeconds = Math.max(0, Math.round(toNumber(legacyMinutesRaw) * 60));
  ...
  const showToast = (message: string, type: 'success' | 'error' | 'info' = 'info', duration: number = 2500) => {
    setToast({
  ...
  const saveComputationToFile = async () => {
    try {
      ...
      if (currentComputationId) {
        ...
        const existingIndex = savedComputations.findIndex((item) => item.name.toLowerCase() === name.toLowerCase());
        if (existingIndex >= 0) {
      ...
      const message = isUpdate || existingIndex >= 0 ? `Updated: ${name}` : `Saved: ${name}`;
  ...
  const confirmCreateNewComputation = () => {
    ...
    if (savedComputations.some((item) => item.name.toLowerCase() === nextName.toLowerCase())) {
      showToast('A saved computation with that name already exists', 'error');
      return;
    }
  ...
  const confirmRenameComputation = async () => {
    ...
    if (savedComputations.some((item) => item.id !== renameComputationId && item.name.toLowerCase() === nextName.toLowerCase())) {
      showToast('A saved computation with that name already exists', 'error');
      return;
    }
  ...
  const deleteComputation = async (id: string) => {
    try {
      const name = savedComputations.find((item) => item.id === id)?.name || 'Unknown';
  ...
  const reset = () => {
    ...
    setIsLootModalOpen(false);
    setIsMapModalOpen(false);
    setSaveStatus('Reset values.');
  };

  const updateDraftInvestment = (key: keyof Omit<InvestmentEntry, 'id'>, value: string | Currency) => {
  ...
  const updateDraftLoot = (key: keyof Omit<InvestmentEntry, 'id'>, value: string | Currency) => {
  ...
  const addInvestment = () => {
    ...
    if (!itemName || ratioItemAmount <= 0 || ratioCostAmount <= 0 || quantity <= 0) {
  ...
  const addMap = () => {
    ...
    const completionMinutes = Math.max(0, Math.floor(toNumber(draftMap.completionMinutes)));
    const completionSeconds = Math.min(59, Math.max(0, Math.floor(toNumber(draftMap.completionSeconds))));
    const quantity = toNumber(draftMap.quantity);

    if (!mapName || (completionMinutes <= 0 && completionSeconds <= 0) || quantity <= 0) {
  ...
  const updateInvestmentField = (id: string, key: keyof Omit<InvestmentEntry, 'id'>, value: string | Currency) => {
    setState((previous) => ({
      ...previous,
  ...
  const updateLootField = (id: string, key: keyof Omit<InvestmentEntry, 'id'>, value: string | Currency) => {
    setState((previous) => ({
      ...previous,
  ...
  const updateMapField = (id: string, key: keyof Omit<MapEntry, 'id'>, value: string) => {
    setState((previous) => ({
      ...previous,
  ...
  const profitTone = calc.profit > 0 ? '#73C991' : calc.profit < 0 ? '#F14C4C' : semantic.textPrimary;
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          ...
          <View style={styles.headerMetaArea}>
            <ThemedText style={[styles.headerCurrentName, { color: semantic.textMuted }]} numberOfLines={1}>
          ...
          <View style={styles.headerActionsRow}>
            <TouchableOpacity
              ...
              style={[styles.headerButton, { borderColor: semantic.border }]}
              accessibilityLabel="Create new computation"
              activeOpacity={0.7}
            >
              <MaterialIcons name="add" size={18} color={semantic.accent} />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <MaterialIcons name="save" size={18} color={semantic.accent} />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <MaterialIcons name="list" size={18} color={semantic.accent} />
            </TouchableOpacity>

            <TouchableOpacity
              onPress={reset}
              style={[styles.headerButton, { borderColor: semantic.border }]}
              accessibilityLabel="Reset values"
              activeOpacity={0.7}
            >
              <MaterialIcons name="restart-alt" size={18} color={semantic.accent} />
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.contentArea}>
          ...
          >
            ...
            <ThemedText style={[styles.rateHint, { color: semantic.textMuted }]}>
              {isRateLoading
            ...
            <View style={styles.rateInputRow}>
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    borderColor: semantic.border,
                    backgroundColor: state.useLiveRate ? semantic.accent + '22' : 'transparent',
                  },
                ]}
                accessibilityLabel="Apply live PoE Ninja rate"
                activeOpacity={0.7}
              >
                <ThemedText style={[styles.useLiveButtonText, { color: semantic.accent }]}>Use Live</ThemedText>
              </TouchableOpacity>
            </View>

            <View style={styles.sectionHeaderRow}>
              <ThemedText style={[styles.sectionTitle, { color: semantic.textPrimary }]}>Investment</ThemedText>
              <TouchableOpacity
              ...
              >
                ...
                <ThemedText style={[styles.addButtonText, { color: semantic.accent }]}>Add Investment</ThemedText>
              </TouchableOpacity>
            </View>

            <ThemedText style={[styles.label, { color: semantic.textMuted }]}>Extra Investment (Chaos)</ThemedText>
            <TextInput
              value={state.additionalInvestmentChaos}
              onChangeText={(value) => updateField('additionalInvestmentChaos', value)}
              keyboardType="decimal-pad"
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tableContainer}>
              <View style={[styles.table, { borderColor: semantic.border }]}> 
                <View style={[styles.tableRow, styles.tableHeaderRow, { borderBottomColor: semantic.border }]}> 
                  <ThemedText style={[styles.tableHeaderCell, styles.colItem]}>Item</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colRatio]}>Ratio</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colQty]}>Qty</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colTotalChaos]}>Total (Chaos)</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colTotalDiv]}>Total (Div)</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colActions]}> </ThemedText>
                </View>

                {investmentRows.length === 0 ? (
                  <View style={[styles.tableRow, { borderBottomColor: semantic.border }]}> 
                    <ThemedText style={[styles.tableCell, { color: semantic.textMuted }]}>No investments yet.</ThemedText>
                  </View>
                ) : (
                  investmentRows.map((entry) => (
                    <View key={entry.id} style={[styles.tableRow, { borderBottomColor: semantic.border }]}> 
                      <TextInput
                        value={entry.itemName}
                        onChangeText={(value) => updateInvestmentField(entry.id, 'itemName', value)}
                        placeholder="Item"
                        placeholderTextColor={semantic.textMuted}
                        style={[
                          styles.tableInput,
                      ...
                      <View style={[styles.tableRatioRow, styles.colRatio]}>
                        <TextInput
                          ...
                          onChangeText={(value) => updateInvestmentField(entry.id, 'ratioItemAmount', value)}
                          keyboardType="decimal-pad"
                          placeholder="20"
                          placeholderTextColor={semantic.textMuted}
                          style={[
                        ...
                        />
                        <ThemedText style={[styles.tableCell, { color: semantic.textMuted }]}>:</ThemedText>
                        <TextInput
                          value={entry.ratioCostAmount}
                          onChangeText={(value) => updateInvestmentField(entry.id, 'ratioCostAmount', value)}
                          keyboardType="decimal-pad"
                          placeholder="1.5"
                          placeholderTextColor={semantic.textMuted}
                          style={[
                        ...
                        />
                        <TouchableOpacity
                          onPress={() => updateInvestmentField(entry.id, 'ratioCurrency', entry.ratioCurrency === 'divine' ? 'chaos' : 'divine')}
                          style={[
                        ...
                        >
                          <ThemedText style={[styles.tableCurrencyChipText, { color: semantic.accent }]}>
                            {entry.ratioCurrency === 'divine' ? 'Div' : 'Chaos'}
                          </ThemedText>
                        </TouchableOpacity>
                      </View>
                      ...
                      <TextInput
                        ...
                        onChangeText={(value) => updateInvestmentField(entry.id, 'quantity', value)}
                        keyboardType="decimal-pad"
                        placeholder="Qty"
                        placeholderTextColor={semantic.textMuted}
                        style={[
                      ...
                      <ThemedText style={[styles.tableCell, styles.colTotalChaos]} numberOfLines={1}>
                        {formatChaos(entry.totalChaos)}
                      </ThemedText>
                      <ThemedText style={[styles.tableCell, styles.colTotalDiv]} numberOfLines={1}>
                        {chaosPerDivine > 0 || entry.ratioCurrency === 'divine' ? formatDiv(entry.totalDiv) : 'Set rate'}
                      </ThemedText>
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                      >
                        <MaterialIcons name="close" size={16} color={semantic.textMuted} />
                      </TouchableOpacity>
                    </View>
                  ))
                )}
                ...
                <View style={[styles.tableRow, styles.tableFooterRow]}> 
                  <ThemedText style={[styles.tableFooterCell, styles.colItem]}>Totals</ThemedText>
                  <View style={styles.colRatio} />
                  <View style={styles.colQty} />
                  <ThemedText style={[styles.tableFooterCell, styles.colTotalChaos]}>{formatChaos(tableTotals.totalChaos)}</ThemedText>
                  <ThemedText style={[styles.tableFooterCell, styles.colTotalDiv]}>
                    {chaosPerDivine > 0 ? formatDiv(tableTotals.totalDiv) : 'Set rate'}
                  </ThemedText>
            ...
            <ThemedText style={[styles.investmentSummary, { color: semantic.textMuted }]}>
              Total investment used in calculation: {formatChaos(totalInvestmentChaos)}
            </ThemedText>
          </ThemedView>
          ...
          >
            <View style={styles.sectionHeaderRow}>
              <ThemedText style={[styles.sectionTitle, { color: semantic.textPrimary }]}>Loot</ThemedText>
              <TouchableOpacity
              ...
              >
                <MaterialIcons name="add" size={16} color={semantic.accent} />
                <ThemedText style={[styles.addButtonText, { color: semantic.accent }]}>Add Loot</ThemedText>
              </TouchableOpacity>
            </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tableContainer}>
              <View style={[styles.table, { borderColor: semantic.border }]}> 
                ...
                {lootRows.length === 0 ? (
                  <View style={[styles.tableRow, { borderBottomColor: semantic.border }]}> 
                    <ThemedText style={[styles.tableCell, { color: semantic.textMuted }]}>No loot yet.</ThemedText>
                  </View>
                ) : (
                  lootRows.map((entry) => (
                    <View key={entry.id} style={[styles.tableRow, { borderBottomColor: semantic.border }]}> 
                      <TextInput
                        value={entry.itemName}
                        onChangeText={(value) => updateLootField(entry.id, 'itemName', value)}
                        placeholder="Item"
                        placeholderTextColor={semantic.textMuted}
                        style={[
                          styles.tableInput,
                      ...
                      <View style={[styles.tableRatioRow, styles.colRatio]}>
                        <TextInput
                          value={entry.ratioItemAmount}
                          onChangeText={(value) => updateLootField(entry.id, 'ratioItemAmount', value)}
                          keyboardType="decimal-pad"
                          placeholder="20"
                          placeholderTextColor={semantic.textMuted}
                          style={[
                        ...
                        <TextInput
                          value={entry.ratioCostAmount}
                          onChangeText={(value) => updateLootField(entry.id, 'ratioCostAmount', value)}
                          keyboardType="decimal-pad"
                          placeholder="1.5"
                          placeholderTextColor={semantic.textMuted}
                          style={[
                        ...
                        />
                        <TouchableOpacity
                          onPress={() => updateLootField(entry.id, 'ratioCurrency', entry.ratioCurrency === 'divine' ? 'chaos' : 'divine')}
                          style={[
                            styles.tableCurrencyChip,
                            {
                              borderColor: semantic.border,
                      ...
                      <TextInput
                        value={entry.quantity}
                        onChangeText={(value) => updateLootField(entry.id, 'quantity', value)}
                        keyboardType="decimal-pad"
                        placeholder="Qty"
                        placeholderTextColor={semantic.textMuted}
                        style={[
                ...
                <View style={[styles.tableRow, styles.tableFooterRow]}> 
                  ...
                  <View style={styles.colRatio} />
                  <View style={styles.colQty} />
                  <ThemedText style={[styles.tableFooterCell, styles.colTotalChaos]}>{formatChaos(lootTotals.totalChaos)}</ThemedText>
                  <ThemedText style={[styles.tableFooterCell, styles.colTotalDiv]}>
                    {chaosPerDivine > 0 ? formatDiv(lootTotals.totalDiv) : 'Set rate'}
                  </ThemedText>
                  <View style={styles.colActions} />
                </View>
              </View>
          ...
          >
            <View style={styles.sectionHeaderRow}>
              <ThemedText style={[styles.sectionTitle, { color: semantic.textPrimary }]}>Maps</ThemedText>
              <TouchableOpacity
              ...
              >
                <MaterialIcons name="add" size={16} color={semantic.accent} />
                <ThemedText style={[styles.addButtonText, { color: semantic.accent }]}>Add Map</ThemedText>
              </TouchableOpacity>
            </View>

            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tableContainer}>
              <View style={[styles.mapTable, { borderColor: semantic.border }]}> 
                <View style={[styles.tableRow, styles.tableHeaderRow, { borderBottomColor: semantic.border }]}> 
                  <ThemedText style={[styles.tableHeaderCell, styles.colMapName]}>Map / Fragment</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colMapTime]}>Time/Run (mm:ss)</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colQty]}>Qty</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colTotalMinutes]}>Total Time</ThemedText>
                  <ThemedText style={[styles.tableHeaderCell, styles.colActions]}> </ThemedText>
                </View>

                {mapRows.length === 0 ? (
                  <View style={[styles.tableRow, { borderBottomColor: semantic.border }]}> 
                    <ThemedText style={[styles.tableCell, { color: semantic.textMuted }]}>No maps yet.</ThemedText>
                  </View>
                ) : (
                  mapRows.map((entry) => (
                    <View key={entry.id} style={[styles.tableRow, { borderBottomColor: semantic.border }]}> 
                      <TextInput
                        value={entry.mapName}
                        onChangeText={(value) => updateMapField(entry.id, 'mapName', value)}
                      ...
                      <View style={[styles.mapTimeGroup, styles.colMapTime]}>
                        <TextInput
                          ...
                          onChangeText={(value) => updateMapField(entry.id, 'completionMinutes', value)}
                        ...
                        />
                        <ThemedText style={[styles.mapTimeColon, { color: semantic.textMuted }]}>:</ThemedText>
                        <TextInput
                          value={entry.completionSeconds}
                          onChangeText={(value) => updateMapField(entry.id, 'completionSeconds', value)}
                          keyboardType="number-pad"
                          placeholder="30"
                          placeholderTextColor={semantic.textMuted}
                          style={[
                      ...
                      <TextInput
                        value={entry.quantity}
                        onChangeText={(value) => updateMapField(entry.id, 'quantity', value)}
                        keyboardType="decimal-pad"
                        placeholder="Qty"
                        placeholderTextColor={semantic.textMuted}
                        style={[
                      ...
                      <ThemedText style={[styles.tableCell, styles.colTotalMinutes]} numberOfLines={1}>
                ...
                <View style={[styles.tableRow, styles.tableFooterRow]}> 
                  <ThemedText style={[styles.tableFooterCell, styles.colMapName]}>Totals</ThemedText>
                  <View style={styles.colMapTime} />
                  <ThemedText style={[styles.tableFooterCell, styles.colQty]}>{Math.round(mapTotals.totalQuantity)}</ThemedText>
                  <ThemedText style={[styles.tableFooterCell, styles.colTotalMinutes]}>{formatHmsFromSeconds(mapTotals.totalSeconds)}</ThemedText>
                  <View style={styles.colActions} />
                </View>
              </View>
            </ScrollView>
            ...
            <ThemedText style={[styles.investmentSummary, { color: semantic.textMuted }]}>
              Total map time recorded: {formatHmsFromSeconds(mapTotals.totalSeconds)}
            </ThemedText>
          </ThemedView>

          </ScrollView>

          <View style={[styles.fixedSummaryArea, { borderTopColor: semantic.border }]}> 
            <ThemedView
              style={[
                styles.card,
                {
            ...
            >
              <View style={[styles.summaryTableRow, { borderBottomColor: semantic.border }]}> 
                <ThemedText style={[styles.metricLabel, styles.summaryLabelCell]}>Total Investment</ThemedText>
                <ThemedText style={[styles.metricValue, styles.summaryValueCell]}>
                  {chaosPerDivine > 0 ? formatDiv(totalInvestmentDiv) : 'Set rate'}
                </ThemedText>
                <ThemedText style={[styles.metricValue, styles.summaryValueCell]}>{formatChaos(totalInvestmentChaos)}</ThemedText>
              </View>

              <View style={[styles.summaryTableRow, { borderBottomColor: semantic.border }]}> 
                <ThemedText style={[styles.metricLabel, styles.summaryLabelCell]}>Total Loot</ThemedText>
                <ThemedText style={[styles.metricValue, styles.summaryValueCell]}>
                  {chaosPerDivine > 0 ? formatDiv(calc.totalLootDiv) : 'Set rate'}
                </ThemedText>
                <ThemedText style={[styles.metricValue, styles.summaryValueCell]}>{formatChaos(calc.totalLootChaos)}</ThemedText>
              </View>

              <View style={[styles.summaryTableRow, { borderBottomColor: semantic.border }]}> 
                <ThemedText style={[styles.metricLabel, styles.summaryLabelCell]}>Net Profit</ThemedText>
                <ThemedText style={[styles.metricValue, styles.summaryValueCell, { color: profitTone }]}>
                  {chaosPerDivine > 0 ? formatDiv(calc.profitInDivines) : 'Set rate'}
                </ThemedText>
                <ThemedText style={[styles.metricValue, styles.summaryValueCell, { color: profitTone }]}>{formatChaos(calc.profit)}</ThemedText>
              </View>

              {hasMapData ? (
                <>
                  <View style={[styles.summaryTableRow, { borderBottomColor: semantic.border }]}> 
                    <ThemedText style={[styles.metricLabel, styles.summaryLabelCell]}>Grand Total Map Time</ThemedText>
                    <ThemedText style={[styles.metricValue, styles.summaryValueCell]}>-</ThemedText>
                    <ThemedText style={[styles.metricValue, styles.summaryValueCell]}>{formatHmsFromSeconds(mapTotals.totalSeconds)}</ThemedText>
                  </View>

                  <View style={[styles.summaryTableRow, { borderBottomColor: semantic.border }]}> 
                    <ThemedText style={[styles.metricLabel, styles.summaryLabelCell]}>Profit / Hour</ThemedText>
                    <ThemedText style={[styles.metricValue, styles.summaryValueCell, { color: profitTone }]}>
                      {chaosPerDivine > 0 ? formatDiv(profitPerHourDiv) : 'Set rate'}
                    </ThemedText>
                    <ThemedText style={[styles.metricValue, styles.summaryValueCell, { color: profitTone }]}>{formatChaos(profitPerHourChaos)}</ThemedText>
                  </View>
                </>
              ) : null}
            </ThemedView>

            <ThemedText style={[styles.statusText, { color: semantic.textMuted }]}>{saveStatus}</ThemedText>
          </View>
        </View>
        ...
        >
          <View style={[styles.modalOverlay, { backgroundColor: semantic.overlay }]}> 
            <View style={[styles.modalCard, { backgroundColor: semantic.panelBackground, borderColor: semantic.border }]}> 
              <ThemedText type="subtitle">Add Investment</ThemedText>

              <ThemedText style={[styles.label, { color: semantic.textMuted }]}>Item Name</ThemedText>
              <TextInput
                value={draftInvestment.itemName}
                onChangeText={(value) => updateDraftInvestment('itemName', value)}
              ...
              <ThemedText style={[styles.label, { color: semantic.textMuted }]}>Item : Cost Ratio</ThemedText>
              <View style={styles.modalRatioRow}>
                <TextInput
                  value={draftInvestment.ratioItemAmount}
                  onChangeText={(value) => updateDraftInvestment('ratioItemAmount', value)}
                  keyboardType="decimal-pad"
                  placeholder="e.g. 20"
                  placeholderTextColor={semantic.textMuted}
                  style={[
                ...
                <ThemedText style={[styles.ratioSeparator, { color: semantic.textMuted }]}>:</ThemedText>

                <TextInput
                  value={draftInvestment.ratioCostAmount}
                  onChangeText={(value) => updateDraftInvestment('ratioCostAmount', value)}
                  keyboardType="decimal-pad"
                  placeholder="e.g. 1.5"
                  placeholderTextColor={semantic.textMuted}
                  style={[
                ...
                <View style={styles.currencySwitchRow}>
                  <TouchableOpacity
                    onPress={() => updateDraftInvestment('ratioCurrency', 'divine')}
                    style={[
                      styles.currencyOption,
                      {
                        borderColor: semantic.border,
                        backgroundColor: draftInvestment.ratioCurrency === 'divine' ? semantic.accent + '22' : 'transparent',
                      },
                    ]}
                  >
                    <ThemedText style={{ color: semantic.textPrimary }}>Divine</ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() => updateDraftInvestment('ratioCurrency', 'chaos')}
                    style={[
                      styles.currencyOption,
                      {
                        borderColor: semantic.border,
                        backgroundColor: draftInvestment.ratioCurrency === 'chaos' ? semantic.accent + '22' : 'transparent',
                      },
                    ]}
                  >
                    <ThemedText style={{ color: semantic.textPrimary }}>Chaos</ThemedText>
                  </TouchableOpacity>
                </View>
              </View>

              <ThemedText style={[styles.label, { color: semantic.textMuted }]}>Quantity</ThemedText>
              <TextInput
                value={draftInvestment.quantity}
                onChangeText={(value) => updateDraftInvestment('quantity', value)}
                keyboardType="number-pad"
                placeholder="e.g. 200"
                placeholderTextColor={semantic.textMuted}
                style={[
              ...
              <View style={styles.modalActionsRow}>
                <TouchableOpacity
                  ...
                  style={[styles.modalSecondaryButton, { borderColor: semantic.border }]}
                  activeOpacity={0.7}
                >
                  <ThemedText style={{ color: semantic.textMuted }}>Cancel</ThemedText>
                </TouchableOpacity>

                <TouchableOpacity
                  onPress={addInvestment}
                  style={[styles.modalPrimaryButton, { backgroundColor: semantic.accent }]}
                  activeOpacity={0.7}
                >
                  <ThemedText style={styles.modalPrimaryButtonText}>Add</ThemedText>
                </TouchableOpacity>
              </View>
            </View>
          </View>
            ...
            <View style={[styles.modalCard, { backgroundColor: semantic.panelBackground, borderColor: semantic.border }]}> 
              ...
              <View style={styles.modalRatioRow}>
                <TextInput
                  ...
                  onChangeText={(value) => updateDraftLoot('ratioItemAmount', value)}
                  keyboardType="decimal-pad"
                  placeholder="e.g. 20"
                  placeholderTextColor={semantic.textMuted}
                  style={[
                ...
                <TextInput
                  ...
                  onChangeText={(value) => updateDraftLoot('ratioCostAmount', value)}
                  keyboardType="decimal-pad"
                  placeholder="e.g. 1.5"
                  placeholderTextColor={semantic.textMuted}
                  style={[
                ...
                <View style={styles.currencySwitchRow}>
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      {
                        borderColor: semantic.border,
                        backgroundColor: draftLoot.ratioCurrency === 'divine' ? semantic.accent + '22' : 'transparent',
                      },
                    ]}
                  >
                    <ThemedText style={{ color: semantic.textPrimary }}>Divine</ThemedText>
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      {
                        borderColor: semantic.border,
                        backgroundColor: draftLoot.ratioCurrency === 'chaos' ? semantic.accent + '22' : 'transparent',
                      },
                    ]}
                  >
                    <ThemedText style={{ color: semantic.textPrimary }}>Chaos</ThemedText>
        ...
        >
          <View style={[styles.modalOverlay, { backgroundColor: semantic.overlay }]}> 
            <View style={[styles.modalCard, { backgroundColor: semantic.panelBackground, borderColor: semantic.border }]}> 
              ...
              <ThemedText style={[styles.label, { color: semantic.textMuted }]}>Map / Fragment Name</ThemedText>
              <TextInput
              ...
              <ThemedText style={[styles.label, { color: semantic.textMuted }]}>Time To Complete (min:sec)</ThemedText>
              <View style={styles.modalTimeRow}>
                <TextInput
                  value={draftMap.completionMinutes}
                  onChangeText={(value) => updateDraftMap('completionMinutes', value)}
                  keyboardType="number-pad"
                  placeholder="7"
                  placeholderTextColor={semantic.textMuted}
                  style={[
                ...
                <TextInput
                  ...
                  onChangeText={(value) => updateDraftMap('completionSeconds', value)}
                  keyboardType="number-pad"
                  placeholder="30"
                  placeholderTextColor={semantic.textMuted}
                  style={[
        ...
        <Modal visible={isCreateNameModalOpen} animationType="fade" transparent>
          <View style={[styles.modalOverlay, { backgroundColor: semantic.overlay }]}>
            <View style={[styles.modalCard, { backgroundColor: semantic.panelBackground, borderColor: semantic.border }]}>
              ...
              <ThemedText style={[styles.label, { color: semantic.textMuted, marginTop: 12 }]}>Computation Name</ThemedText>
              <TextInput
              ...
              <View style={styles.modalActionsRow}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <ThemedText style={styles.modalPrimaryButtonText}>Create</ThemedText>
                </TouchableOpacity>
              </View>
            </View>
          </View>
        ...
        <Modal visible={isLoadPickerModalOpen} animationType="slide" transparent>
          <View
            style={[
              styles.modalOverlay,
              {
          ...
          >
            ...
            >
              <View style={styles.savedPickerHeader}>
                <ThemedText style={[styles.modalTitle, { color: semantic.textPrimary }]}>Saved Computations</ThemedText>
                <TouchableOpacity
                  onPress={() => setIsLoadPickerModalOpen(false)}
                  accessibilityLabel="Close saved computations modal"
                  activeOpacity={0.7}
                >
                  <MaterialIcons name="close" size={24} color={semantic.textPrimary} />
                </TouchableOpacity>
              </View>

              {savedComputations.length === 0 ? (
                <ThemedText style={[styles.savedPickerEmpty, { color: semantic.textMuted }]}>No saved computations.</ThemedText>
              ) : (
                <FlatList
                    ...
                    return (
                      <View
                        style={[
                          ...
                          {
                            backgroundColor: isActive ? semantic.accent + '15' : semantic.screenBackground,
                            borderColor: semantic.border,
                          },
                        ]}
                      >
                        <TouchableOpacity
                          ...
                          activeOpacity={0.7}
                        >
                          <ThemedText style={[styles.savedPickerName, { color: semantic.textPrimary }]} numberOfLines={1}>
                            {item.name}
                          </ThemedText>
                          <ThemedText style={[styles.savedPickerMeta, { color: semantic.textMuted }]} numberOfLines={1}>
                        ...
                        <TouchableOpacity
                          ...
                          activeOpacity={0.7}
                        >
                          <MaterialIcons name="edit" size={20} color={semantic.textMuted} />
                        </TouchableOpacity>
                        <TouchableOpacity
                          ...
                          activeOpacity={0.7}
                        >
                          <MaterialIcons name="delete" size={20} color={semantic.textMuted} />
                        </TouchableOpacity>
                      </View>
                    );
                  }}
        ...
        <Modal visible={isRenameModalOpen} animationType="fade" transparent>
          <View style={[styles.modalOverlay, { backgroundColor: semantic.overlay }]}>
            <View style={[styles.modalCard, { backgroundColor: semantic.panelBackground, borderColor: semantic.border }]}>
              ...
              <ThemedText style={[styles.label, { color: semantic.textMuted, marginTop: 12 }]}>New Name</ThemedText>
              <TextInput
              ...
              <View style={styles.modalActionsRow}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <ThemedText style={styles.modalPrimaryButtonText}>Rename</ThemedText>
                </TouchableOpacity>
              </View>
            </View>
          </View>
